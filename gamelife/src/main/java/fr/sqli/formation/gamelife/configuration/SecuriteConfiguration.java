...
import fr.sqli.formation.gamelife.service.authentification.AuthentificationDetailsService;
...
public class SecuriteConfiguration {
    ...
    private final ProprietesCleRsa rsaKeys;
    private final AuthentificationDetailsService authentificationDetailsService;

    private static final String ROLE_ADMIN = "ROLE_ADMIN";
    private static final String ROLE_MODERATEUR = "ROLE_MODERATEUR";

    public SecuriteConfiguration(ProprietesCleRsa rsaKeys, AuthentificationDetailsService pAuthentificationDetailsService) {
        this.rsaKeys = rsaKeys;
        this.authentificationDetailsService = pAuthentificationDetailsService;
    }

    @Bean
    public AuthenticationManager authManager(AuthentificationDetailsService pAuthentificationDetailsService) {
        var authProvider = new DaoAuthenticationProvider();
        authProvider.setUserDetailsService(pAuthentificationDetailsService);
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests(authorize -> authorize.requestMatchers(new AntPathRequestMatcher(h2Url + "/**") // h2
                    ...
                    header.frameOptions(HeadersConfigurer.FrameOptionsConfig::disable);
                        ...
                        .antMatchers("/api/v1/plateformes").permitAll()//hasAnyRole(ROLE_ADMIN, ROLE_MODERATEUR)
                        .antMatchers("/api/v1/plateformes/{produitId}").permitAll()//hasAnyRole(ROLE_ADMIN, ROLE_MODERATEUR)
                        .antMatchers("/api/v1/categories").permitAll()//hasAnyRole(ROLE_ADMIN, ROLE_MODERATEUR)
                        .antMatchers("/api/v1/categories/{produitId}").permitAll()//hasAnyRole(ROLE_ADMIN, ROLE_MODERATEUR)
                ...
                .userDetailsService(authentificationDetailsService);
        return http.build();
    }
